...
QtObject {
    ...
    function updateFilteredModel() {
        ...
        for (var i = 0; i < appModel.count; i++) {
            ...
            if (!item.iconPath && !item.thumb && !item.customIcon)
                continue;
            ...
            if (same) {
                for (var u = 0; u < results.length; u++) {
                    ...
                    if (f.background !== r.background)
                        filteredModel.setProperty(u, "background", r.background);

                    if (f.displayName !== r.displayName)
                        filteredModel.setProperty(u, "displayName", r.displayName);

                    if (f.customIcon !== r.customIcon)
                        filteredModel.setProperty(u, "customIcon", r.customIcon);

                    if (f.thumb !== r.thumb)
                        filteredModel.setProperty(u, "thumb", r.thumb);

                    if (f.tags !== r.tags)
                        filteredModel.setProperty(u, "tags", r.tags);
    ...
    function _applyAppsConfig() {
        ...
        for (var i = 0; i < appModel.count; i++) {
            ...
            if (item.background !== bg)
                appModel.setProperty(i, "background", bg);

            if (item.displayName !== dn)
                appModel.setProperty(i, "displayName", dn);

            if (item.customIcon !== ci)
                appModel.setProperty(i, "customIcon", ci);

            if (item.tags !== tags)
                appModel.setProperty(i, "tags", tags);

            if (item.hidden !== hidden)
                appModel.setProperty(i, "hidden", hidden);
