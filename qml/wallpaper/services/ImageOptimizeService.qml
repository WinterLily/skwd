...
QtObject {
    ...
    readonly property string _trashDir: Config.wallCacheDir + "/wallpaper/trash/images"
    readonly property string _stagingDir: Config.wallCacheDir + "/wallpaper/staging"
    property var _queue: []
    ...
    function _buildQueue() {
        ...
        for (var i = 0; i < rows.length; i++) already[rows[i].src] = {
            "preset": rows[i].preset,
            "format": rows[i].format
        }
        var allFiles = _scanStdout.filter(function(l) {
            return l.trim() !== "";
        });
        var queue = [];
        var skippedCount = 0;
    ...
    function _recordOptimization(src, dest, parts) {
        ...
        DbService.exec("INSERT INTO image_optimize(src,dest,preset,format,width,height,orig_size,new_size,optimized_at) VALUES(" + DbService.sqlStr(src) + "," + DbService.sqlStr(dest) + "," + DbService.sqlStr(_currentPreset) + ",'webp'," + newW + "," + newH + "," + origSize + "," + newSize + "," + now + ") ON CONFLICT(src) DO UPDATE SET dest=excluded.dest,preset=excluded.preset,format=excluded.format," + "width=excluded.width,height=excluded.height,orig_size=excluded.orig_size,new_size=excluded.new_size,optimized_at=excluded.optimized_at;");
        if (src !== dest) {
            var oldKey = src.split("/").pop();
            var newKey = dest.split("/").pop();
            DbService.exec("UPDATE meta SET key=" + DbService.sqlStr(newKey) + "," + "name=" + DbService.sqlStr(dest.split("/").pop()) + " WHERE key=" + DbService.sqlStr(oldKey) + ";");
        }
    }
    ...
    function _recordSkip(src, parts) {
        ...
        var h = parts[3] || "0";
        var now = Math.floor(Date.now() / 1000);
        DbService.exec("INSERT INTO image_optimize(src,dest,preset,format,width,height,orig_size,new_size,optimized_at) VALUES(" + DbService.sqlStr(src) + "," + DbService.sqlStr(src) + "," + DbService.sqlStr(_currentPreset) + ",'skip'," + w + "," + h + "," + origSize + "," + origSize + "," + now + ") ON CONFLICT(src) DO UPDATE SET preset=excluded.preset,optimized_at=excluded.optimized_at;");
    }
    ...
    function cleanTrash() {
        if (!Config.autoDeleteImageTrash)
            return ;
