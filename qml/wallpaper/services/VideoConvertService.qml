...
QtObject {
    ...
    function _launchAutoConvertWorker(src) {
        ...
        var finalDest = isWE ? Config.videoDir + "/" + destName.replace(/\.mp4$/, "-" + hashStr + ".mp4") : src;
    ...
    function _scanVideos() {
        ...
        var script = 'find ' + vidDir + ' -type f ' + ImageService.findExtPattern(ImageService.videoExtensions.filter(function(e) {
            return e !== "gif";
        })) + ' 2>/dev/null\n' + 'if [ -d ' + weDir + ' ]; then\n' + '  find ' + weDir + ' -type f ' + ImageService.findExtPattern(ImageService.videoExtensions.filter(function(e) {
            return e !== "gif";
    ...
    function _buildQueue() {
        ...
        for (var i = 0; i < rows.length; i++) already[rows[i].src] = rows[i].preset
    ...
    function _recordConversion(src, parts, finalDest) {
        ...
        DbService.exec("INSERT INTO video_convert(src,dest,preset,codec,width,height,orig_size,new_size,converted_at) VALUES(" + DbService.sqlStr(dest) + "," + DbService.sqlStr(dest) + "," + DbService.sqlStr(_currentPreset) + ",'hevc'," + newW + "," + newH + "," + origSize + "," + newSize + "," + now + ") ON CONFLICT(src) DO UPDATE SET dest=excluded.dest,preset=excluded.preset,codec=excluded.codec," + "width=excluded.width,height=excluded.height,orig_size=excluded.orig_size,new_size=excluded.new_size,converted_at=excluded.converted_at;");
        if (dest !== src && Config.weDir) {
            var weId = src.replace(Config.weDir + "/", "").split("/")[0];
            if (weId)
                DbService.exec("DELETE FROM meta WHERE we_id=" + DbService.sqlStr(weId));
    ...
    function _recordSkip(src, parts) {
        ...
        var now = Math.floor(Date.now() / 1000);
        DbService.exec("INSERT INTO video_convert(src,dest,preset,codec,width,height,orig_size,new_size,converted_at) VALUES(" + DbService.sqlStr(src) + "," + DbService.sqlStr(src) + "," + DbService.sqlStr(_currentPreset) + "," + DbService.sqlStr(codec) + "," + w + "," + h + "," + origSize + "," + origSize + "," + now + ") ON CONFLICT(src) DO UPDATE SET preset=excluded.preset,converted_at=excluded.converted_at;");
    }
    ...
    function cleanTrash() {
        if (!Config.autoDeleteVideoTrash)
            return ;
