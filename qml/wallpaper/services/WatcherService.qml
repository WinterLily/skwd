...
QtObject {
    ...
    readonly property var _imageExts: {
        ...
        ImageService.imageExtensions.forEach(function(e) {
            o[e] = 1;
        });
        return o;
    }
    readonly property var _videoExts: {
        ...
        ImageService.videoExtensions.forEach(function(e) {
            o[e] = 1;
        });
        return o;
    }
    ...
    function start() {
        if (watching || !Config.configLoaded || !wallpaperDir)
            return ;
    ...
    function notifyRenamed(oldName, newName) {
        if (_known[oldName]) {
            delete _known[oldName];
            _known[newName] = true;
        }
        if (_knownVid[oldName]) {
            delete _knownVid[oldName];
            _knownVid[newName] = true;
        }
    }
    ...
    function _buildCommand() {
        ...
        script += "_watch_dirs=()\n";
        for (var i = 0; i < dirs.length; i++) script += "[ -d " + _sq(dirs[i]) + " ] && _watch_dirs+=(" + _sq(dirs[i]) + ")\n"
    ...
    function _handleEvent(event, dir, name) {
        ...
        if (event.indexOf("CREATE") >= 0 && event.indexOf("CLOSE_WRITE") < 0)
            return ;
