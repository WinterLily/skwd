...
QtObject {
    ...
    readonly property string cacheDir: Config.wallCacheDir + "/wallpaper"
    ...
    function _loadCache() {
        ...
        for (var i = 0; i < rows.length; i++) {
            if (rows[i].matugen) {
                try {
                    _cache[rows[i].key] = JSON.parse(rows[i].matugen);
                } catch (e) {
                }
            }
        }
    ...
    function _buildQueue() {
        ...
        for (var i = 0; i < files.length; i++) {
            var path = files[i];
            var key = (path.indexOf(service.thumbsDir) === 0 || path.indexOf(service.weThumbsDir) === 0) ? DbService.cacheKey(path) : path.split("/").pop();
            if (seen[key] || _cache[key])
                continue;
    ...
    function _processMatugenOutput(stdout, item) {
        ...
        try {
            ...
            for (var key in colors) {
                ...
                for (var j = 1; j < parts.length; j++) camelKey += parts[j].charAt(0).toUpperCase() + parts[j].substring(1)
                var value = colors[key];
                if (typeof value === "object" && value !== null) {
                    var dark = value.dark || value["default"] || "#888888";
                    if (typeof dark === "object")
                        dark = dark.color || "#888888";
