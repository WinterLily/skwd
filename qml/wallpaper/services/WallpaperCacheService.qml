...
QtObject {
 ...
 // Delete WE entries from meta table
    ...
    function processWeItem(weId, weItemDir) {
        ...
        var cmd = "cd " + DbService.shellQuote(dir) + " && " + "title=$(jq -r '.title // \"Unknown\"' project.json 2>/dev/null || echo Unknown); " + "preview=''; for p in preview.jpg preview.png preview.gif; do [ -f \"$p\" ] && preview=\"$p\" && break; done; " + "echo \"$title\"; echo \"$preview\"";
    ...
    function removeFiles(files) {
        ...
        DbService.exec("DELETE FROM meta WHERE key IN (" + keys.map(function(k) {
    ...
    function _drainIncrementalQueue() {
        while (_incrementalActive < maxJobs && _incrementalQueue.length > 0) {
            ...
            var proc = _incrementalWorkerComponent.createObject(service, {
                "command": cmd,
                "_item": item
            });
            proc.running = true;
    ...
    function _buildIncrementalCmd(item) {
        ...
        if (item.type === "static") {
            ...
            if (isWebp) {
                ...
                genThumb = "if head -c 1024 " + src + " 2>/dev/null | grep -q ANIM; then " + "echo ANIMWEBP:yes; " + ImageService.animatedWebpThumbnailCmd(srcFrame0, thumb) + "; " + "else " + ImageService.thumbnailCmd(src, thumb) + "; fi";
            } else {
                genThumb = ImageService.thumbnailCmd(src, thumb);
            }
            thumbCmd = genThumb + "; " + "[ -f " + thumb + " ] && " + ImageService.thumbnailCmd(thumb, thumbSm, ImageService.smallThumbWidth, ImageService.smallThumbHeight, ImageService.smallThumbQuality) + "; " + "[ -f " + thumb + " ] && " + ImageService.hueExtractCmd(thumb);
        } else if (item.type === "we") {
            var weD = DbService.shellQuote(item.src.replace(/\/preview\.[^/]+$/, ""));
            thumbCmd = "we_dir=" + weD + "; " + "video_file=\"\"; " + "if [ -f \"$we_dir/project.json\" ]; then " + "  we_type=$(jq -r '.type // \"scene\"' \"$we_dir/project.json\" 2>/dev/null | tr '[:upper:]' '[:lower:]'); " + "  if [ \"$we_type\" = \"video\" ]; then " + "    proj_file=$(jq -r '.file // \"\"' \"$we_dir/project.json\" 2>/dev/null); " + "    if [ -n \"$proj_file\" ] && [ -f \"$we_dir/$proj_file\" ]; then " + "      video_file=\"$we_dir/$proj_file\"; " + "    fi; " + "  fi; " + "fi; " + "if [ -z \"$video_file\" ]; then " + "  video_file=$(find \"$we_dir\" -type f \\( -iname '*.mp4' -o -iname '*.webm' \\) ! -iname 'preview.*' -print -quit 2>/dev/null); " + "fi; " + "if [ -n \"$video_file\" ]; then " + "  " + ImageService.videoThumbnailCmd('"$video_file"', thumb, 2) + "; " + "fi; " + "[ ! -f " + thumb + " ] || [ ! -s " + thumb + " ] && " + ImageService.thumbnailCmd(src, thumb) + "; " + "[ -f " + thumb + " ] && " + ImageService.thumbnailCmd(thumb, thumbSm, ImageService.smallThumbWidth, ImageService.smallThumbHeight, ImageService.smallThumbQuality) + "; " + "echo \"VIDEOFILE:$video_file\"; " + "[ -f " + thumb + " ] && " + ImageService.hueExtractCmd(thumb);
        } else {
            thumbCmd = ImageService.videoThumbnailCmd(src, thumb, 0) + "; " + "[ -f " + thumb + " ] && " + ImageService.thumbnailCmd(thumb, thumbSm, ImageService.smallThumbWidth, ImageService.smallThumbHeight, ImageService.smallThumbQuality) + "; " + "[ -f " + thumb + " ] && " + ImageService.hueExtractCmd(thumb);
        }
    ...
    function _filterWorkItems() {
        ...
        for (var ek in _existingCache) {
            if (!seenKeys[ek])
                staleKeys.push(_existingCache[ek].key);
    ...
    function _buildItemCmd(item) {
        ...
        if (item.type === "static") {
            ...
            return ["sh", "-c", genThumb + "; " + "[ -f " + thumb + " ] && " + ImageService.thumbnailCmd(thumb, thumbSm, ImageService.smallThumbWidth, ImageService.smallThumbHeight, ImageService.smallThumbQuality) + "; " + "[ -f " + thumb + " ] && " + ImageService.hueExtractCmd(thumb)];
        } else if (item.type === "video") {
            return ["sh", "-c", ImageService.videoThumbnailCmd(src, thumb, 0) + "; " + "[ -f " + thumb + " ] && " + ImageService.thumbnailCmd(thumb, thumbSm, ImageService.smallThumbWidth, ImageService.smallThumbHeight, ImageService.smallThumbQuality) + "; " + "[ -f " + thumb + " ] && " + ImageService.hueExtractCmd(thumb)];
        } else if (item.type === "we") {
            var weD = DbService.shellQuote(item.src.replace(/\/preview\.[^/]+$/, ""));
            return ["sh", "-c", "we_dir=" + weD + "; " + "video_file=\"\"; " + "if [ -f \"$we_dir/project.json\" ]; then " + "  we_type=$(jq -r '.type // \"scene\"' \"$we_dir/project.json\" 2>/dev/null | tr '[:upper:]' '[:lower:]'); " + "  if [ \"$we_type\" = \"video\" ]; then " + "    proj_file=$(jq -r '.file // \"\"' \"$we_dir/project.json\" 2>/dev/null); " + "    if [ -n \"$proj_file\" ] && [ -f \"$we_dir/$proj_file\" ]; then " + "      video_file=\"$we_dir/$proj_file\"; " + "    fi; " + "  fi; " + "fi; " + "if [ -z \"$video_file\" ]; then " + "  video_file=$(find \"$we_dir\" -type f \\( -iname '*.mp4' -o -iname '*.webm' \\) ! -iname 'preview.*' -print -quit 2>/dev/null); " + "fi; " + "if [ -n \"$video_file\" ]; then " + "  " + ImageService.videoThumbnailCmd('"$video_file"', thumb, 2) + "; " + "fi; " + "[ ! -f " + thumb + " ] || [ ! -s " + thumb + " ] && " + ImageService.thumbnailCmd(src, thumb) + "; " + "[ -f " + thumb + " ] && " + ImageService.thumbnailCmd(thumb, thumbSm, ImageService.smallThumbWidth, ImageService.smallThumbHeight, ImageService.smallThumbQuality) + "; " + "echo \"VIDEOFILE:$video_file\"; " + "[ -f " + thumb + " ] && " + ImageService.hueExtractCmd(thumb)];
        }
        return ["true"];
    }
    ...
    function _writeOneResult(e) {
        ...
        DbService.exec("INSERT INTO meta(key,type,name,thumb,thumb_sm,video_file,we_id,mtime,hue,sat) VALUES(" + DbService.sqlStr(key) + "," + DbService.sqlStr(e.type) + "," + DbService.sqlStr(e.name) + "," + DbService.sqlStr(e.thumb) + "," + DbService.sqlStr(e.thumbSm || "") + "," + DbService.sqlStr(e.videoFile || "") + "," + DbService.sqlStr(e.id || "") + "," + (parseInt(e.mtime) || 0) + "," + (e.group != null ? parseInt(e.group) : 99) + "," + (parseInt(e.sat) || 0) + ") ON CONFLICT(key) DO UPDATE SET type=excluded.type,name=excluded.name,thumb=excluded.thumb,thumb_sm=excluded.thumb_sm,video_file=excluded.video_file,we_id=excluded.we_id,mtime=excluded.mtime,hue=excluded.hue,sat=excluded.sat;");
    }
