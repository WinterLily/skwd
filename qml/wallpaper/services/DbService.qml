...
QtObject {
    ...
    function execBatch(sqlArray) {
        _db.transaction(function(tx) {
            for (var i = 0; i < sqlArray.length; i++) tx.executeSql(sqlArray[i])
        });
    }
    ...
    function query(sql) {
        ...
        _db.readTransaction(function(tx) {
            ...
            for (var i = 0; i < rs.rows.length; i++) results.push(rs.rows.item(i))
        });
        return results;
    }
    ...
    Component.onCompleted: {
        _db = LocalStorage.openDatabaseSync("skwd-wall", "1.0", "skwd-wall cache", 5e+06);
        try {
            _db.transaction(function(tx) {
