...
Scope {
    ...
    PanelWindow {
        ...
        screen: Quickshell.screens.find((s) => {
            return s.name === notifScope.mainMonitor;
        }) ?? Quickshell.screens[0]
        implicitWidth: notifScope.centerOpen ? (screen ? screen.width : 1920) : notifScope.popupWidth + notifScope.popupRightMargin * 2
        implicitHeight: notifScope.centerOpen ? (screen ? screen.height : 1080) : Math.max(1, notifScope.effectiveTopMargin + popupColumn.childrenRect.height + notifScope.popupSpacing * 2)
        color: "transparent"
        ...
        Item {
            ...
            Canvas {
                ...
                function animate() {
                    centerBorderAnim.restart();
                }

                function reset() {
                    centerBorderAnim.stop();
                    progress = 0;
                    requestPaint();
                }
                ...
                onPaint: {
                    ...
                    var segments = [{
                        ...
                        "x2": width,
                        "y2": height,
                        "len": Math.sqrt(s * s + height * height)
                    }, {
                        "x1": width,
                        "y1": height,
                        "x2": s,
    ...
    component NotificationCard: Item {
        ...
        Canvas {
            ...
            Connections {
                function onLineProgressChanged() {
                    cardBorder.requestPaint();
                }
