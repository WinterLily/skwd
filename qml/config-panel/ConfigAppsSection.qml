    ...
    Repeater {
        ...
        Rectangle {
            ...
            property bool _matchesSearch: {
                if (!root.panel._appSearchFilter)
                    return true;

                var q = root.panel._appSearchFilter;
                if (appKey.toLowerCase().indexOf(q) >= 0)
                    return true;

                var entry = appEntry;
                if (entry.displayName && entry.displayName.toLowerCase().indexOf(q) >= 0)
                    return true;

                if (entry.tags && entry.tags.toLowerCase().indexOf(q) >= 0)
                    return true;
            ...
            Connections {
                function onAppsDataChanged() {
                    ...
                    appCard.appEntry = obj ? Object.assign({
                    }, obj) : {
                    };
                }
                ...
                Rectangle {
                    ...
                    Row {
                        ...
                        Text {
                            text: {
                                var parts = [];
                                if (appCard.appEntry.displayName)
                                    parts.push(appCard.appEntry.displayName);

                                if (appCard.appEntry.icon)
                                    parts.push("icon: " + appCard.appEntry.icon);

                                if (appCard.appEntry.hidden)
                                    parts.push("hidden");
                ...
                Column {
                    ...
                    ConfigTextField {
                        ...
                        onEdited: (v) => {
                            if (!root.panel.appsData[appCard.appKey])
                                root.panel.appsData[appCard.appKey] = {
                            };

                            if (v === "")
                                delete root.panel.appsData[appCard.appKey].displayName;
                            else
                                root.panel.appsData[appCard.appKey].displayName = v;
                            ...
                            Rectangle {
                                ...
                                TextInput {
                                    ...
                                    onTextEdited: {
                                        ...
                                        if (text === "")
                                            delete root.panel.appsData[appCard.appKey].icon;
                                        else
                                            root.panel.appsData[appCard.appKey].icon = text;
                                        root.panel.hasUnsavedChanges = true;
                                        root.panel.appsDataChanged();
                                    }
                    ...
                    ConfigTextField {
                        ...
                        onEdited: (v) => {
                            ...
                            if (v === "")
                                delete root.panel.appsData[appCard.appKey].tags;
                            else
                                root.panel.appsData[appCard.appKey].tags = v;
                            root.panel.hasUnsavedChanges = true;
                            root.panel.appsDataChanged();
                        }
                        colors: root.colors
                    ...
                    ConfigToggle {
                        ...
                        onToggled: (v) => {
                            ...
                            if (!v)
                                delete root.panel.appsData[appCard.appKey].hidden;
                            else
                                root.panel.appsData[appCard.appKey].hidden = v;
                            root.panel.hasUnsavedChanges = true;
                            root.panel.appsDataChanged();
                        }
                        colors: root.colors
                                    ...
                                    TextInput {
                                        ...
                                        onTextEdited: {
                                            ...
                                            if (text === "")
                                                ...
                                                root.panel.appsData[appCard.appKey].background = text;
                                            root.panel.hasUnsavedChanges = true;
                                            root.panel.appsDataChanged();
                                        }
