...
Item {
    ...
    Canvas {
        ...
        onPaint: {
            ...
            case "top-right":
                ...
                ctx.moveTo(0, 0); ctx.lineTo(width, 0);
                ctx.lineTo(width, height); ctx.lineTo(s, height);
                break;
            case "bottom-left":
                // Bottom full-width, top-right corner cut — slant on the right
                ctx.moveTo(0, 0); ctx.lineTo(width - s, 0);
            ...
            if (root.accentEdges) {
                ...
                case "top-left":
                    ...
                    ctx.moveTo(0, height); ctx.lineTo(width - s, height); ctx.lineTo(width, 0);
                    break;
                case "top-right":
                    // Left slant + bottom edge (bottom-left diagonal)
                    ctx.moveTo(0, 0); ctx.lineTo(s, height); ctx.lineTo(width, height);
                    break;
                case "bottom-left":
                    // Top edge + right slant (top-right diagonal)
                    ctx.moveTo(0, 0); ctx.lineTo(width - s, 0); ctx.lineTo(width, height);
                    break;
                default: // bottom-right
                    // Left slant + top edge (top-left diagonal)
                    ctx.moveTo(0, height); ctx.lineTo(s, 0); ctx.lineTo(width, 0);
                    break;
                }
        ...
        Connections {
            target: root
            function onAccentEdgesChanged() { bg.requestPaint(); }
            function onCornerChanged() { bg.requestPaint(); }
        }
    }
    ...
    Row {
        ...
        anchors.horizontalCenterOffset: (root.corner === "top-right" || root.corner === "bottom-right")
            ? root.diagSlant / 2 : -root.diagSlant / 2
