...
QtObject {
    ...
    // Public values
    // ──────────────────────────────────────────────────
    property real cpuUsage: 0   // percent 0–100
    property real memUsage: 0   // percent 0–100
    property real gpuUsage: 0   // percent 0–100 (AMD sysfs)
    ...
    property var cpuHwmonNameProbe: FileView {
    ...
    property var cpuTempPollTimer: Timer {
        id: cpuTempPollTimer
        interval: 5000; repeat: true; running: false; triggeredOnStart: true
    ...
    property var gpuHwmonNameProbe: FileView {
    ...
    property var amdBusyProbe: FileView {
        id: amdBusyProbe
        ...
        onLoadFailed: {
            var next = cardIdx + 1
            if (next < 4) probe(next)
        }
    }

    property var gpuTempPollTimer: Timer {
        id: gpuTempPollTimer
        interval: 5000; repeat: true; running: false; triggeredOnStart: true
        onTriggered: gpuTempFile.reload()
    }

    property var gpuTempFile: FileView {
        id: gpuTempFile
    ...
    property var gpuBusyPollTimer: Timer {
        id: gpuBusyPollTimer
        interval: 3000; repeat: true; running: false; triggeredOnStart: true
        onTriggered: gpuBusyFile.reload()
    }

    property var gpuBusyFile: FileView {
        id: gpuBusyFile
