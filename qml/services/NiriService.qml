...
Item {
    ...
    function getWindowPosition(layout) {
        ...
        return { "x": floatingWindowPosition, "y": floatingWindowPosition }
    }
    ...
    function toSortedWindowList(windowList) {
        return windowList.map(win => {
            ...
            if (a.window.position.x !== b.window.position.x) return a.window.position.x - b.window.position.x
            if (a.window.position.y !== b.window.position.y) return a.window.position.y - b.window.position.y
    ...
    function handleWindowOpenedOrChanged(eventData) {
        try {
            ...
            const previouslyFocusedId = focusedWindowIndex >= 0 && focusedWindowIndex < windows.length
                ? windows[focusedWindowIndex].id : null
            ...
            if (newWindow.isFocused) {
                ...
                if (previouslyFocusedId !== null && previouslyFocusedId !== windowData.id) {
                    const oldFocused = windows.find(w => w.id === previouslyFocusedId)
                    if (oldFocused) oldFocused.isFocused = false
                }
                activeWindowChanged()
            }
    ...
    function getActiveOutput() {
        ...
        for (var i = 0; i < workspaces.length; i++) {
            if (workspaces[i].isFocused) return workspaces[i].output
        }
        if (workspaces.length > 0) return workspaces[0].output
        return null
    }
